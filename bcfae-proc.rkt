...
;; aSto : number BCFAE-Value Store -> Store
(define (aSto location value store)
  (lambda (want-loc)
    (if (= location want-loc)
        value
        (store-lookup want-loc store))))

;; mtSto : -> Store
(define (mtSto)
  (lambda (want-loc)
    (error 'store-lookup "no value at location")))

;; store-lookup : number Store -> BCFAE-Value
(define (store-lookup want-loc store)
  (store want-loc))
